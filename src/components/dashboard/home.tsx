"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { BookOpen } from "lucide-react";

import { Button } from "@/components/ui/button";
import { publicUrl } from "@/lib/public-path";
import type { Snapshot, Timeline } from "@/lib/types";

import { Legend } from "./legend";
import { MatrixTable } from "./matrix-table";
import { MobileCards } from "./mobile-cards";
import { StatusBar } from "./status-bar";

const emptyTimeline: Timeline = { dates: [], assets: {} };

export function HomeDashboard() {
  const [snapshot, setSnapshot] = useState<Snapshot | null>(null);
  const [timeline, setTimeline] = useState<Timeline>(emptyTimeline);
  const [refreshEnabled, setRefreshEnabled] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const [snapRes, tlRes, healthRes] = await Promise.all([
          fetch(publicUrl("/latest.json"), { cache: "no-store" }),
          fetch(publicUrl("/timeline.json"), { cache: "no-store" }),
          fetch(publicUrl("/api/health"), { cache: "no-store" }).catch(() => null),
        ]);
        if (!snapRes.ok) throw new Error(`快照读取失败 (${snapRes.status})`);
        const snap = (await snapRes.json()) as Snapshot;
        const tl = tlRes.ok ? ((await tlRes.json()) as Timeline) : emptyTimeline;
        if (cancelled) return;
        setSnapshot(snap);
        setTimeline(tl);
        if (healthRes && healthRes.ok) {
          const health = (await healthRes.json()) as { refresh?: boolean };
          setRefreshEnabled(Boolean(health.refresh));
        }
      } catch (err) {
        if (!cancelled) setError(err instanceof Error ? err.message : "加载失败");
      } finally {
        if (!cancelled) setLoading(false);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, []);

  return (
    <main className="mx-auto flex w-full max-w-[1200px] flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">
            方向偏好 <span className="text-slate-400">×</span> 波动率偏好
          </h1>
          <p className="mt-1 max-w-3xl text-sm text-slate-500">
            六大类资产的 DELTA（短线 1–4 周 / 长线 1–3 月）与 VEGA · SKEW · GAMMA · VOL 体制信号矩阵。 数据来自 Yahoo Finance 期货日线与 ETF 期权链、CBOE 波动率指数、CFTC 持仓报告，全部为公开免费来源。
          </p>
        </div>
        <Button variant="outline" size="sm" render={<Link href="/methodology" />}>
          <BookOpen />
          方法说明与复现
        </Button>
      </header>

      {loading && (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">
          正在读取最新快照…
        </div>
      )}

      {error && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-800">
          {error}
        </div>
      )}

      {!loading && !error && !snapshot && (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center">
          <h2 className="text-lg font-semibold text-slate-800">还没有数据快照</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-slate-500">
            运行 <code className="rounded bg-slate-100 px-1">python -m macrovol.cli run</code> 生成快照。
          </p>
        </div>
      )}

      {snapshot && (
        <>
          <StatusBar snapshot={snapshot} historyCount={timeline.dates.length || 1} refreshEnabled={refreshEnabled} />
          {!refreshEnabled && (
            <p className="text-[11px] text-slate-500">
              本页由 GitHub Pages 托管：每个工作日美东收盘后自动重算并发布。要立刻更新，到仓库的 Actions 里手动跑 “Update and publish”。
            </p>
          )}
          <div className="hidden md:block">
            <MatrixTable snapshot={snapshot} timeline={timeline} />
          </div>
          <div className="md:hidden">
            <MobileCards snapshot={snapshot} timeline={timeline} />
          </div>
          <div className="grid gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs text-slate-600 shadow-sm sm:grid-cols-2">
            <div>
              <span className="font-semibold text-slate-800">短线：</span>
              {snapshot.notes.short}
            </div>
            <div className="sm:border-l sm:border-slate-200 sm:pl-4">
              <span className="font-semibold text-slate-800">长线：</span>
              {snapshot.notes.long}
            </div>
          </div>
          <Legend />
        </>
      )}
    </main>
  );
}
